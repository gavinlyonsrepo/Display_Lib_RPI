/*!
	...
	@note
			Only tested on 4 digit + decimal points
	@test
...
bool Setup(void)
{
	printf("Test Begin :: Model 4B :: TM1637plus\r\n");
...
void Test2(void)
{
	...
	tm.DisplayDecimalwDot(1, 0x10, false, 4, 0); // "   1."
	delaySecRDL(myTestDelay);
	tm.DisplayDecimalwDot(23, 0x20, false, 4, 0); //"  2.3"
	delaySecRDL(myTestDelay);
	tm.DisplayDecimalwDot(784, 0x40, false, 4, 0); //" 7.84"
	delaySecRDL(myTestDelay);
	tm.DisplayDecimalwDot(6281, 0x80, false, 4, 0);  // "6.281"
	delaySecRDL(myTestDelay);

	printf("Test 2D: decimal number leading zeros on ,  with semi:colon on\r\n");
	tm.DisplayDecimalwDot(1, 0x10, true, 4, 0); // "0001."
	delaySecRDL(myTestDelay);
	tm.DisplayDecimalwDot(23, 0x20, true, 4, 0); // "002.3"
	delaySecRDL(myTestDelay);
	tm.DisplayDecimalwDot(784, 0x40, true, 4, 0); // "07.84"
	delaySecRDL(myTestDelay);
	tm.DisplayDecimalwDot(6281, 0x80, true, 4, 0); //"6.281"
	delaySecRDL(myTestDelay);
	tm.displayClear();
}
...
void Test3(void)
{
	...
	char min[] = "07";
	char sec[] = "53";
	tm.DisplayString(min, 0x40, 2, 0); // Display "07." starting at position 0
	tm.DisplayString(sec, 0x00, 2, 2); // Display "53" starting at position 2
	delaySecRDL(myTestDelay);
	...
	tm.DisplayString(string, 0xF0, NUM_OF_DIGITS, 0); // Display "O.P.E.N." starting at position 0
	delaySecRDL(myTestDelay);
	tm.displayClear();
